...
const leafRanks=[2,7,0,5,3,6,1,4];
...
const checks=[
 ...
 a=>{const p=a.map(v=>Math.floor(v/2));const adjacent=[0,1].some(off=>[0,2,4,6].every(i=>p[(i+off)%8]===p[(i+off+1)%8]));const opposite=[0,1,2,3].every(i=>p[i]===p[i+4]);const size=a.map(v=>leafRanks[v]);const circular=[1,7].some(step=>size.every((v,i)=>i===0||v===(size[i-1]+step)%8));return [adjacent,opposite,circular]},
...
root.PuzzleRules={check,lengths,leafRanks,latin,strokes,sizes,connection};if(typeof module!=='undefined')module.exports=root.PuzzleRules;
